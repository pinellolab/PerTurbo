...
def main(argv: list[str] | None = None) -> None:
    ...
    # Stage one is finished. Keep its raw controls on the host for downstream
    # gene slices; retaining the full device panel wastes space needed by CRT.
    # Mutate the existing object so controls_loaded sees the same host array.
    controls.counts = np.array(controls.counts, copy=True)
    ...
    if crt_accumulator is not None:
        ...
        if "crt_low_information" in crt_columns:
            ...
            # gene, not one element being thin. Quantified over the tested rows
            # only, because the flag itself is: a control-anchored screen leaves
            # its control elements untested and therefore unflagged, and asking
            # for every row would report zero such genes on every screen that
            # has one.
            tested_rows = np.asarray(crt_accumulator.tested, dtype=bool)
            genes_all_flagged = (
                int(np.count_nonzero(flagged[tested_rows].all(axis=0)))
                if flagged.size and tested_rows.any()
                else 0
            )
