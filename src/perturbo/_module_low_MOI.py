...
class PerTurboModule(PyroBaseModuleClass):
    def __init__(
        ...
        **module_kwargs,
        ...
        self._guide.append(
            AutoNormal(
                poutine.block(self.model, expose=["element_effects"]), init_loc_fn=init_to_median, init_scale=0.05
            )
        )
        self._guide.append(
            AutoDelta(
                poutine.block(
                    self.model, expose=["element_factors", "factor_gene_loadings", "latent_factors", "latent_loadings"]
                ),
                init_loc_fn=init_to_median,
            )
        )
        ...
        if self.n_factors is not None:
            ...
            self.register_buffer("factor_loading_prior_scale", torch.tensor(0.1))
        ...
        if self.n_factors is not None:
            ...
            with factor_plate_2, gene_plate:
                factor_x_gene = pyro.sample(
                    "factor_gene_loadings",
                    dist.Laplace(0.0, self.factor_loading_prior_scale),
                )
